...
def check_cli_requirement(md: Metadata) -> int:
    if md.current_cli_requirement.specifier != f"=={md.current_lib_static_version}":
        logger.error(
            ...
            f"which is out of sync with nonos' version (expected =={md.current_lib_static_version})",
        )
        return 1
    else:
...
def check_requires_python(md: Metadata) -> int:
    if md.cli_requires_python != md.lib_requires_python:
        logger.error(
            f"nonos-cli's python requirement ({md.cli_requires_python}) "
            f"is out of sync with nonos' ({md.lib_requires_python})",
        )
        return 1
    else:
...
def main() -> int:
    ...
    return (
        check_static_version(md) + check_cli_requirement(md) + check_requires_python(md)
    )
